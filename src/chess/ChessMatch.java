...
public class ChessMatch {
	...
	private boolean testCheckMate(Color color) {
		...
		for(Piece p : list){
			...
			for(int i = 0; i < board.getRows(); i ++) {
				for(int j =0; j < board.getColumns(); j ++){
					//checando se isso é um movimento possivel e tira do checkMate
					if(mat[i][j]){
						//pega a peça movimenta e ve se é possivel se mover para a caasa especificada e depois a retorna para sua origem
